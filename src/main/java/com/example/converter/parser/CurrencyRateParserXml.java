...
import com.example.converter.model.Currencies;
...
public class CurrencyRateParserXml implements CurrencyRateParser {
    ...
    public CurrencyRate filterCurrencyRate(ArrayList<CurrencyRate>arrayList, String curRate){
        return arrayList.stream()
                .filter(currencyRate -> currencyRate.charCode.equals(curRate))
                .findFirst()
                .orElse(null);
    }
    public  BigDecimal convertStringToBI(String out){

        String correctOut = out.replace(",",".");
        return  new BigDecimal(correctOut);
    }
}